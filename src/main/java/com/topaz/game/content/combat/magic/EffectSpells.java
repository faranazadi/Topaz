    ...
    public static boolean handleSpell(Player player, int button) {
        ...
        switch (spell.get()) {
            ...
            case VENGEANCE:
                ...
                if (player.getVengeanceTimer().secondsRemaining() >= 1) {
                    player.getPacketSender().sendMessage("You can only cast Vengeance once every 30 seconds.");
                    return true;
                }
                ...
                player.performAnimation(new Animation(4411)); //4410
                player.performGraphic(new Graphic(725, GraphicHeight.HIGH)); //726
                break;
        }
        return true;
    }
